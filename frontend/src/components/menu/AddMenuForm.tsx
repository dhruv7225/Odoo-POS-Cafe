...
export const AddMenuForm: React.FC<AddMenuFormProps> = ({ onAdd, onCancel }) => {
  ...
  const addVariant = () => {
    setVariants([...variants, { id: Date.now().toString(), name: "", priceOption: 0 }]);
  };
  ...
  const handleSubmit = () => {
    ...
    // Ensure all variants have names
  ...
  return (
    <Card className="rounded-xl shadow-sm border-border/60">
      ...
      <CardContent className="space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between border-b pb-2">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Add sizes, addons or customizations.</p>
            </div>
            <Button type="button" variant="outline" size="sm" onClick={addVariant} className="text-primary hover:text-primary hover:bg-primary/5">
          ...
          {variants.length === 0 ? (
            <div className="py-4 text-center text-sm text-muted-foreground bg-muted/20 border border-dashed rounded-lg">
              No variants added for this item.
            </div>
          ) : (
            <div className="space-y-3">
              {variants.map((v) => (
                <div key={v.id} className="flex items-start gap-4 p-3 border rounded-lg bg-accent/20">
                  ...
                  <div className="flex-1 space-y-1.5">
                    <Label className="text-xs">Added Price (+)</Label>
                    <div className="relative">
                      ...
                      <Input 
                        ...
                        placeholder="0.00" 
                        value={v.priceOption} 
                        onChange={e => updateVariant(v.id, "priceOption", parseFloat(e.target.value) || 0)} 
                        className="h-9 pl-6"
                      />
                    </div>
                  </div>
        ...
        {/* Toggles */}
        <div className="grid md:grid-cols-2 gap-6 pt-2 border-t border-border/50 mt-6">
      ...
      <CardFooter className="flex justify-end gap-3 bg-muted/20 border-t py-4 px-6">
        ...
        <Button onClick={handleSubmit} className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-white shadow-md">
          Save Item
        </Button>
      </CardFooter>
    </Card>
